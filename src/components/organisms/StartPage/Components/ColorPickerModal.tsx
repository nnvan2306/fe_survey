...
const ColorPickerModal: React.FC<ColorPickerModalProps> = ({
    ...
    return (
        <div
            ...
            } fixed inset-0 flex items-center justify-center z-50 p-4 backdrop-blur-sm transition-opacity duration-300 ${
                open
                    ? "opacity-100 ease-out"
                    : "opacity-0 ease-in pointer-events-none"
            }`}
        >
            <div className={`${styles["modal-container"]} w-full max-w-lg`}>
                <div
                    className={`flex items-center justify-between px-6 py-4 ${styles["modal-header"]}`}
                >
                    <h2 className={`${styles["modal-title"]}`}>CHỌN MÀU NỀN</h2>
                </div>

                <div className="px-6 py-8">
                    <div className="flex flex-col items-center">
                        <span className="text-gray-600 mb-4">
                            Nhấn vào mã màu để đổi
                        </span>
                        <div className="flex space-x-4 w-full">
                            <div
                                className={`flex transition-all duration-300 ${
                                    isGradientMode ? "flex-1" : "w-[80%]"
                                }`}
                            >
                                <label
                                    className={`relative flex items-center justify-between py-2 shadow-md cursor-pointer ${
                                        isGradientMode
                                            ? "rounded-l-none rounded-r-full pr-4"
                                            : "rounded-full px-4"
                                    }`}
                            ...
                            >
                                <label
                                    className={`relative flex items-center justify-between py-2 shadow-md cursor-pointer ${
                                        isGradientMode
                                            ? "rounded-l-full rounded-r-none pl-4"
                                            : "rounded-full px-4"
                                    }`}
                ...
                <div
                    className={`flex justify-end px-6 py-4 ${styles["modal-footer"]}`}
                >
                    <button
